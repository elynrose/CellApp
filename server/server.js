...
async function makeAPIRequest(provider, endpoint, data = null, apiKey = null) {
  return new Promise(async (resolve, reject) => {
    ...
    // Construct URL properly - ensure endpoint path is correctly joined with baseUrl
    let fullUrl;
    if (endpoint.startsWith('http://') || endpoint.startsWith('https://')) {
      // Endpoint is already a full URL
      fullUrl = endpoint;
    } else {
      // Join baseUrl and endpoint, ensuring proper path joining
      const baseUrlObj = new URL(config.baseUrl);
      // Ensure endpoint starts with / if it doesn't already
      const endpointPath = endpoint.startsWith('/') ? endpoint : '/' + endpoint;
      // Join paths properly - remove trailing slash from base path if present
      const basePath = baseUrlObj.pathname === '/' ? '' : (baseUrlObj.pathname.endsWith('/') ? baseUrlObj.pathname.slice(0, -1) : baseUrlObj.pathname);
      fullUrl = `${baseUrlObj.protocol}//${baseUrlObj.host}${basePath}${endpointPath}${baseUrlObj.search}`;
    }
